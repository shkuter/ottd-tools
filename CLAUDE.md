...
- Названия **типов путей** — тот же механизм, третий словарь: `railtypes.ru.json`, ключ —
  четырёхсимвольный лейбл (`RAIL`, `ELRL`, `NAAN`…), общий для обоих наборов, потому что
  лейбл означает один и тот же путь, кто бы его ни поставлял. Часть имён — **наши**, а не
...
`activeRailtype(game, label)` — выбранный путь с откатом на `RAIL`, если набор сменился.
...
- **Мощность и тяга зависят от пути.** `engine/tracktypes.ts`: `vehiclePowerOn()` берёт
  источник по иерархии набора (OHLE > METRO > BATTERY_HYBRID > DIESEL > STEAM), причём OHLE
  доступен только на пути с `catenary`; тяговое усилие считается лишь для машин с ненулевой
  мощностью на этом пути (`ground_vehicle.cpp`: `if (current_power > 0)`).
