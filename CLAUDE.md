...
- Правки поверх перевода — только орфография, регистр и ё, в `pipeline/ru_overrides.json`;
  ...
  экстрактор падает.
- Названия **машин** словаря не имеют: ни Iron Horse, ни ваниль русских имён не дают, и в
  игре они тоже английские — подпись берётся из `train.name` как есть. Появится набор со
  своей локалью — словарь заводится тем же способом, что словарь путей.
