...
- [ ] 1.1 В `pipeline/extract_iron_horse.py` добавить хелпер, повторяющий запись свойства 0x09
      в NML (`n = round(mph * 5000 / 1397)`, затем подгонка на ±1, пока `(10 * n) // 16 == mph`),
      и писать в машину `speed_internal` и `speed_lgv_internal` (`None`, если скорости нет).
      Проверка: `pipeline/.venv/bin/python -m pipeline.extract_iron_horse` отрабатывает, у машины
      со 112 mph получается 180.
- [ ] 1.2 Эталоны в `pipeline/tests/test_known_values.py`: внутренняя скорость для быстрой машины
      (112 mph → 180) и для медленной (45 mph → 73), плюс инвариант `(10 * internal) // 16 == mph`
      для всех машин. Проверка: `pipeline/.venv/bin/python -m unittest pipeline.tests.test_known_values`.
- [ ] 1.3 Прогнать `make data` и убедиться, что `git diff web/src/data/trains.json` содержит
      только новые поля скорости, а остальные значения не поехали.

## 2. Веб: типы и конверсия

- [ ] 2.1 Пробросить внутреннюю скорость в веб-слой: поле в `web/src/types.ts` и маппинг в
...
- [ ] 2.2 В `web/src/engine/units.ts` добавить `internalToKmh()` по формуле игры
      (`Math.floor(Math.trunc(internal * 10 * 1.609344) / 16)`) с ссылкой на `strings.cpp`.
- [ ] 2.3 Тесты конверсии в `web/src/engine/__tests__/engine.test.ts`: 180 → 181 км/ч,
      64 → 64 км/ч (Kirby Paul Tank), 144 → 90 миль/ч. Проверка:
      `cd web && npx vitest run -t "units"`.

## 3. Настройка

- [ ] 3.1 В `web/src/state/settingsStore.ts` добавить `speedUnit: 'imperial' | 'metric'` рядом с
...
- [ ] 3.2 Строки интерфейса в `web/src/i18n/en.json` и `ru.json`: единица «км/ч» / «km/h»,
...
- [ ] 3.3 Переключатель в `web/src/features/settings/SettingsPage.tsx` — `Select` в блоке
      `settings.display` сразу после валюты. Проверка: в `make dev` настройка видна, выбор
      сохраняется после перезагрузки страницы.

## 4. Вывод скорости

- [ ] 4.1 В `web/src/components/format.ts` добавить `speed(internal)`: выбирает конверсию по
      `speedUnit` и подставляет единицу через `t()`; строка собирается в момент вызова.
- [ ] 4.2 В `web/src/engine/consist.ts` заменить `speedLimitMph` / `balancingSpeedMph` /
...
- [ ] 4.3 В `web/src/engine/optimize.ts` заменить `loadedSpeedMph` / `emptySpeedMph` /
...
- [ ] 4.4 Перевести на `speed()` все места вывода: колонка «Скорость» и три показателя состава в
...
- [ ] 5.1 Тест на требование «единица не влияет на расчёт»: переключение `speedUnit` не меняет
      доход рейса, время рейса и порядок выдачи оптимизатора. Кейс в
      `web/src/engine/__tests__/settings-effect.test.ts` при этом не заводится — настройка живёт
      вне `GameSettings`/`CalcSettings`; проверить, что этот тест остаётся зелёным без правок.
- [ ] 5.2 Проверить в браузере (`make dev`) обе системы единиц на всех четырёх вкладках: числа
      совпадают с игрой на контрольной машине (112 миль/ч ↔ 181 км/ч), единицы нигде не смешаны.
- [ ] 5.3 Запись в `CHANGELOG.md` в секцию `[Unreleased]` (на английском): новая настройка и
      смена значения по умолчанию на км/ч.
- [ ] 5.4 Финальный прогон: `make data` (данные менялись) и `make test`, затем `make verify`.
